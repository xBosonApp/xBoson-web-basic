(function()
{
  ...
  // 打开选择模板按钮
  //
  Sidebar.prototype.addPaletteSelectorPanel = function() {
    ...
    //
  	...
  	var skip = { pid2:1, aws2:1, search:1, pid:1 };
  	...
  	mxEvent.addListener(input, 'click', function() {
  	  ...
  	  left.style = 'padding: 10px 0px; overflow-y: scroll; width: 200px;';
  	  var right = document.createElement('div');
  	  right.style = 'min-width: 500px; overflow-y: scroll;';
  	  var img = document.createElement('img');
  	  
  	  right.appendChild(img);
  	  thiz.container.appendChild(dialog);
  	  ...
  	  for (var i = 0; i < conf.length; ++i) {
  	    ...
  	    if (i % 15 == 14) {
  	      line.style = 'border-bottom: 1px solid #555';
  	    }
  	    line.style.paddingLeft = '30px';
  	    // 类型按钮
  	    var item = document.createElement('a');
  	    item.style = 'display: inline-block; margin-left: 10px';
  	    item.innerHTML = mxResources.get(id) || id;
  	    item.href = '#';
  	    // 选择框
  	    var check = document.createElement('input');
  	    check.type = 'checkbox';
  	    check.checked = thiz.isEntryVisible(id);
  	    selector.push({
  	      input : check,
  	      id    : id,
  	    });
  	  ...
  	  mxEvent.addListener(close, 'click', _close);
  	  
  	  function _close() {
  	    dialog.remove();
  	    var str = [];
  	    selector.forEach(function(s) {
  	      if (s.input.checked) str.push(s.id);
  	    });
  	    thiz.showEntries(str.join(';'));
  	  }
  	});
  	
  	function showImage(shown, imgName, link, line) {
  	  mxEvent.addListener(link, 'click', function() {
  	    if (current_select) {
  	      current_select.style.backgroundColor = '#FFF';
  	    }
  	    line.style.backgroundColor = '#CCC';
  	    current_select = line;
  	    shown.src = 'sidebar/images/sidebar-'+ imgName +'.png';
  	  });
  	}
  };
})();