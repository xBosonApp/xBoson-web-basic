...
jQuery(function($) {
  var pop = window.pop;
  $('.xboson').animate({'letter-spacing': '20px', 'opacity':'1'}, 5000);
  
  try {
    var canvas = make_boson($('.show_boson'));
    $(canvas).css({
      'position': 'fixed',
      'top': '100px',
      'left': '0',
      'z-index': '-1',
    }).animate({'opacity':'0.15'}, 3000);
    setTimeout(pop.begin_fadein, 2200);
  } catch(e) {
    console.warn("无法绘制动画", e);
    pop.begin_fadein();
  }
  
  
  function make_boson(target_dom) {
    class Point {
      constructor(x, y) {
        this.pos = {
          x: x,
    ...
    target_dom.append(canvas)
    // document.body.appendChild();
    let W = (canvas.width = target_dom.width()),
        H = (canvas.height = target_dom.height());
