...
(function (H) {
	...
	Chart.prototype.addSeriesAsDrilldown = function (point, ddOptions, levelNumber) {
		this.addSingleSeriesAsDrilldown(point, ddOptions, levelNumber);
		this.applyDrilldown();
	};
	Chart.prototype.addSingleSeriesAsDrilldown = function (point, ddOptions, levelNumber) {
		...
		levelNumber = levelNumber || oldSeries.levelNumber || 0;
		...
		each(oldSeries.chart.series, function (series) {
			if (series.xAxis === xAxis) {
				...
				series.levelNumber = levelNumber || 0;
			}
		});
			...
			if (level.levelNumber === levelToRemove) {
				each(level.levelSeries, function (series) {
					if (series.levelNumber === levelToRemove) { // Not removed, not added as part of a multi-series drilldown
                      try {
						series.remove(false);
                      } catch(E) {console.log('drill ser', E, series.name)}
					}
				});
			}
		});
	...
	wrap(H.Point.prototype, 'init', function (proceed, series, options, x) {
		...
		if (point.drilldown) {
			...
			H.addEvent(point, 'click', function () {
                // console.log('xxx', point.doDrilldown, point)
				point.doDrilldown();
			});
