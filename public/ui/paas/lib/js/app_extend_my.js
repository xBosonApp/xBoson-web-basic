zy.net.loadHtmlSync = function(url, rcb) {
  $.ajax({
    // 与引用这个脚本的页面的路径相同
    url: 'ribbon.html',
...
zy.net.loadHtmlSync('ribbon.html', function(err, html) {
  $('#ribbon').append(html);
});
...
$("#smart-styles > a").bind("click", function () {
    ...
    localStorage.setItem('zy_user_set_style', $this.attr("id"));
});

jQuery(function() {
  var initstyle = localStorage.getItem('zy_user_set_style');
  if (initstyle) {
    $('#' + initstyle).trigger('click');
  }
});
