  (function ($, zy, ide_zride_mod) {
    ...
    function _dom(_id) {
            ...
            function _formDom(_c) {
                ...
                _sbtn.click(function () {
                    ...
                    if (_editor && _path) {
                        ...
                        function saveFile(){
                          var _ts = $.extend(true, _t, zy.g.comm);
                          _ts.mod = ide_zride_mod;
                          Put('/webide/file',_ts,function(msg){
        ...
        function _header(_c) {
            ...
            function _previewbtn() {
                ...
                _btn.click(function () {
                    ...
                    var _u = 'http://'+location.host+'/web/'+ide_zride_mod+'/';
                    _u += 'main.htm?p=' + src;
                    zy.log('window.open  _u == '+_u);
                    window.open(_u, '');
                    
                    // var _tt = {};
                    // _tt = $.extend(true, _tt, zy.g.comm);
                    // _tt.path = src;
                    // _tt.mod = ide_zride_mod;
                    // Get('/webide/preview', _tt, 
                    // function(msg){
                    //   window.open(msg, '');
                    // })
                });
        ...
        function _treeDom() {
            ...
            _tree.append(_tools._label('div').attr('style','overflow: auto; height: 440px;').addClass('row').css('margin-top', '-15px').append(_ul));
        ...
        function _searchByContentResult(_container, _flg) {
            ...
            function _searchApi(_btn, _in) {
                ...
                _btn.click(function () {
                    ...
                    var _ts = $.extend(true, _t, zy.g.comm);
                    _ts.mod = ide_zride_mod;
                    Get('/webide/search',_ts,function(msg){
        ...
        function _searchByidResult(_container, _flg) {
            ...
            function _searchApi(_btn) {
                ...
                _btn.click(function () {
                    var _t = {};
                    _t = $.extend(true, _t, zy.g.comm);
                    _t.content = _input.val();
                    _t.mod = ide_zride_mod;
                    Get('/webide/search',_t,function(msg){
                            _ulSearch.siblings().remove();
                            _ulSearch.empty();
                            if (msg.children.length === 0)
                                _ulSearch.parent().append(_tools._label('div').css({
        ...
        function _filehistory(_container, _flg){
                  ...
                  function searchhistory(){
                    ...
                    var path=basePathOnServer+'/web/'+ide_zride_mod+filepath;
    ...
    function _treeClass(_d, _data) {
        ...
        function _editModal(_node) {
            ...
            if (/^(png)$|^(jpg)$|^(jpeg)$|^(gif)$|^(htm)$|^(html)$|^(js)$|^(css)$|^(woff2)$|^(woff)$|^(ttf)$|^(svg)$|^(eot)$/.test(_lname)){
              _lastname=_lname
            }else{
              _lastname='文件夹'
            }
        ...
        function _dbClick(_e, _id, _node) {
          _node.path = _node.path.replace("/"+ide_zride_mod+"/","/");
          if(_node.isParent){
            var _tagert = $('#zy_tabs').closest('.row').children(':last');
            ...
            if(!$(_a).attr('title')){
                ...
                var _previewbtn = $('#widget-grid').find('header .glyphicon-play').parent();
                    ...
                    if (/^(png)|(jpg)|(jpeg)|(gif)/.test(filelname)){
                        ...
                        _path = '/web/'+ide_zride_mod+'/' + _node.path.substring(1);
                        var _pre = _tab.add(_name, _node.path, _id,filelname,_path);
                        _pre.inputc.val('');
    
                    }else{
                      var _ts = $.extend(true, _tgt, zy.g.comm);
                      _ts.mod = ide_zride_mod;
                      Get('/webide/file',_ts,function(msg){
                          ...
                          if (msg.filetype == 'htm' || msg.filetype == 'html') {
                              _previewbtn.show();
                          } else {
                              _previewbtn.hide();
                          }
    ...
    function _tabs(_d) {
        ...
        function _add(_name, _contentid, _id,filelname,_path) {
            ...
            var _height = $('#jarviswidget-fullscreen-mode').length > 0 ? '500px' : '440px';
  ...
  function Get(url, param, scb, ecb) {
    $.ajax({
      ...
      error: function (XMLHttpRequest, textStatus, errorThrown) {
        zy.ui.msg("服务器请求失败: ", parseParam(param) + "\r\n" + errorThrown, "e");
        zy.log("zride.get.error: " + XMLHttpRequest + "\r\n" + textStatus + "\r\n" + errorThrown);
        ecb && ecb();
      }
    });
  }
