 (function () {
    ...
    function ToolBar(){
      ...
      function SearchBtn(){
        ...
        i.unbind('click').bind('click',function(){
          var target = article.find('#module_con').children('form');
  ...
  /* 与模块对接处 */
  function LoadModule(con,param){zy.log("2222222222222222222");
    var modulecon = con.find('#module_con');
    var header = con.children('.jarviswidget').children('header').find('h2');
    
    modulecon.empty();

    new Module(modulecon, param, function(conn,thiz){zy.log("444444444444444444");
      var mod = thiz.fn.GetSelf();
      modulecon.children('form').hide();
      header.html(mod.modnm);zy.log("555555555555 "+mod.modnm);
    });
  }
