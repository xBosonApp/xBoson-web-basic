/**
 * Created by wyy on 2014/8/12.
 */
/**
 * 部门信息
 * @class MgtDeptList
 */
MgtDeptList = (function() {

  var PT = MgtDeptList.prototype;
  var thiz;
  var conditions;
  /**
   * 默认配置参数
  ...
  //表格元素对象
  var dt = $('#mgt_sys_mdm_getdeptlist_dt');
   ...
   * @constructor
   */
  function MgtDeptList() {
    Console.log("new MgtDeptList()");
    thiz = this;
    thiz.Init();
    thiz.Select();
    return this;
  ...
  PT.Init = function() {
    ...
    thiz.DataTable();
    thiz.Toolbar();
    $('#mgt_sys_mdm_getdeptlist_edit').btnDisable(true);
    $('#mgt_sys_mdm_getdeptlist_delete').btnDisable(true);
  };
  /*窗口改变时，重新调整画面大小*/
  ...
  PT.DataTable = function(data) {
    ...
    var columns = [{
      ...
      "render": function(data, type, row, meta) {
        ...
        return '<a href=\"javascript:void(0);\" rel=\"popover\" data-placement=\"left\" data-original-title=\"<i class=\'fa fa-fw fa-pencil\'></i> 修改状态\" data-content=\"<form id=\'mgt_sys_mdm_getdeptlist_status_form\' onsubmit=\'return false;\' ' + _data + ' style=\'min-width:170px\'>' + str + '<div class=\'form-actions\'><div class=\'row\'><div class=\'col-md-12\'><button id=\'mgt_sys_mdm_getdeptlist_status\' class=\'btn btn-primary btn-sm\' type=\'submit\'>保存</button></div></div></div></form>\" data-html=\"true\">' + zy.cache.cd2name("ZR.0001", row.status) + '</a>';
      }
    }];
    //预设初始化参数
    var options = {
  ...
  PT.Toolbar = function() {
    ...
    dt.on('click', 'tr', function(e) {
      ...
      if (!$(this).find('div').hasClass('popover') && $(this).hasClass('active')) {
        ...
        $('#mgt_sys_mdm_getdeptlist_edit').btnDisable(false);
      }
    });

    //查询
    $('#mgt_sys_mdm_getdeptlist_search').click(function() {
      conditions = $('#mgt_sys_mdm_getdeptlist_form').serialize();
      $('#mgt_sys_mdm_getdeptlist_edit').btnDisable(true);
      $('#mgt_sys_mdm_getdeptlist_search').button('loading');
      thiz.Pagination(1);
    });
    //添加
    $('#mgt_sys_mdm_getdeptlist_add').click(function() {
      thiz._g.param.flg = 'i'; //设置添加模式标记
      zy.net.loadHTML("mgt/sys_mdm/sys_dept/add_upd_dept.html", $("#mgt_sys_mdm_getdeptlist_form2"));
    });
    //修改
    $('#mgt_sys_mdm_getdeptlist_edit').click(function() {
      // 当前选择行 index
  ...
  PT.Pagination = function(page) {
    ...
    $.jqPaginator('#mgt_sys_mdm_getdeptlist_pagination', {
  ...
  PT.SetDt = function(page) {
    ...
    var cb = function(msg) {
      $('#mgt_sys_mdm_getdeptlist_search').button('reset');
      if (msg) {
        $('#mgt_sys_mdm_getdeptlist_edit').btnDisable(true);
        // $('#mgt_sys_mdm_getdeptlist_delete').btnDisable(true);
        ...
        if (msg.count > 0 && msg.result.length > 0) {
          $('#mgt_sys_mdm_getdeptlist_pagination').jqPaginator('option', {
            totalCounts: thiz._g.count,
            pageSize: zy.g.page.pagesize,
            currentPage: page
          });
          ...
          thiz._g.count = 1;
          thiz._g.page = 1;
          $('#mgt_sys_mdm_getdeptlist_pagination').jqPaginator('destroy');
        }
  ...
  PT.Select = function() {
    ...
    zy.net.get('api/orgselect2', function(msg) {
      if (msg) {
        $("#mgt_sys_mdm_getdeptlist_form input[name=orgid]").zySelectCustomData('', true, {
          width: '100%'
        }, msg.result);
      }
    });
    ...
    zy.net.get('api/deptselect2', function(msg) {
      if (msg) {
        $("#mgt_sys_mdm_getdeptlist_form input[name=higher_deptid]").zySelectCustomData('', true, {
          width: '100%'
        }, msg.result);
      }
    });
    ...
    var cb = function() {
      // 字典数据绑定
      $("#mgt_sys_mdm_getdeptlist_form input[name=status]").zySelect('ZR.0001', true, {
        width: '100%'
      });
      $("#mgt_sys_mdm_getdeptlist_form input[name=status]").select2('val', '1');
    };
    // 预处理该画面所需的字典类型，以 , 号分割
