...
AppContol = (function() {
  ...
  PT.Init = function() {
    ...
    $('#application_control_set').btnDisable(true);
  };
  /*窗口改变时，重新调整画面大小*/
  ...
  PT.Toolbar = function() {
    ...
    dt.on('click', 'tr', function(e) {
      ...
      if (!$(this).find('div').hasClass('popover') && $(this).hasClass('active')) {
        ...
        $('#application_control_set').btnDisable(false);
      }
    });
    //查询
    $('#application_control_search').click(function() {
      $('#application_control_set').btnDisable(true);
      $('#application_control_search').button('loading');
      thiz.Pagination(1);
    });
    //修改权限
    $('#application_control_set').click(function() {
      // 当前选择行数据
      ...
      zy.net.loadHTML("yunying_ptgl/application/application_manage.html", $("#application_control_form"));
    });
  };
  ...
  PT.Pagination = function(page) {
    ...
    $.jqPaginator('#application_control_pagination', {
  ...
  PT.SetDt = function(page) {
    ...
    var cb = function(msg) {
      $('#application_control_search').button('reset');
      if (msg) {
        ...
        if (msg.count > 0 && msg.result.length > 0) {
          $('#application_control_pagination').jqPaginator('option', {
            totalCounts: thiz._g.count,
            pageSize: zy.g.page.pagesize,
            currentPage: page
          });
          ...
          thiz._g.count = 1;
          thiz._g.page = 1;
          // $('#application_control_pagination').jqPaginator('destroy');
        }
    ...
    zy.g.am.mod = "jgsqyy";
    $("#application_control_search_form input[name=org_id]").val(thiz._g.param.org_id);
    zy.net.get("api/orgapp_view", cb, $('#application_control_search_form').serialize(), page);
  };
  ...
  PT.Select = function() {
    ...
    var cb = function() {
      // 字典数据绑定
      $("#application_control_search_form input[name=org_type]").zySelect('ZR.0034', true, {
        width: '100%'
      });
    };
    // 预处理该画面所需的字典类型，以 , 号分割
    $("#application_control_search_form input[name=org_type]").val('v');
