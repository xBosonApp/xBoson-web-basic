...
var fetchDataForColumntime = function(data) {
    ...
    var keyArray = [];
    $.each(data[0], function(key, value) {
        if ('月' !== key && '重点人群累计健档人数' !== key && '建档管理累计人数（糖尿病）' !== key && '建档管理累计人数（精神病）' !== key && '建档管理累计人数（高血压）' !== key && '累计规范建档率' !== key && '纸制档案累计人数' !== key && '纸制累计建档率' !== key && '计算机档案规范管理累计人数' !== key && '健康管理人数（儿童）' !== key && '健康（规范）管理人数' !== key && '上一年度末常住人口总数' !== key && '中医药健康管理人数' !== key  && '应管理人数（糖尿病）' !== key && '应管理人数（高血压）' !== key) {
            keyArray.push(key);
        }
    });
    for (var i = 0; i < keyArray.length; i++) {
