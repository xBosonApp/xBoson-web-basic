 ...
 *  var _m = new swdkh(options); // 设置参数选项
 *  _m.m; // Model数据对象
 */
swdkh = (function () {

  var pt = swdkh.prototype;
  ...
  pt.v = {
    ...
    el: el,
    // 配置模式视图
    setup: el.find('#nytg-chartCanvas'),
  };
   ...
   */
  function swdkh(options) {
    zy.log('new swdkh(options)');
    // 合并初始化参数选项
  ...
  var initData = {
    ...
    did: function () {
      zy.extend.get({
        ...
        pt.v.did.zySelectCustomData('', false, {width: '200px', allowClear: false}, msg.data);
      }, {});
    }
  };
  ...
  var events = {
    ...
    select_items: function () {
      ...
      items.find('.style-0').on('change', function(e){
        ...
        ele.closest('.dd-item').data('alias_name',ele.val());
        zy.log('项目别名修改是发生.val() = ' + ele.val());
      });
      // 项目设置时发生
  ...
  var fn = {
        ...
        $.each(pt.m.sel_whe_columns, function(i, t) {
          $.each(t.column_info, function(j, c) {
            if (c.is_select=="1") {
              ...
              var pullLeft2 = $('<div class="pull-left"><label><span>&nbsp;&nbsp;<b>AS</b></span>&nbsp;<input class="style-0" type="text" value="' + c.column_name + '" placeholder="设置项目别名"></label></div>');
              var pullRight = $('<div class="pull-right"><button class="btn btn-success btn-xs dropdown-toggle" data-toggle="dropdown"> 图表 <span class="caret"></span></button></div>');
              var dropdown_menu_ul = $('<ul class="dropdown-menu"></ul>');
              var dropdown_menu_li1 = $('<li><span class="font-xs">&nbsp;项目对应的图表设置：</span></li>');
              dropdown_menu_ul.append(dropdown_menu_li1);
    ...
    where: function () {
      ...
      // where 区域
      var contact = pt.m.sel_where['contact'];zy.log('-------------1-------------'+JSON.stringify(contact));
      var params = pt.m.sel_where['params'];zy.log('------------2--------------'+JSON.stringify(params));
      for (var i = 0, l = contact.length; i < l; i++) {zy.log('-------------11-------------'+JSON.stringify(contact[i]));
        fn.whereJoin(contact[i]);
      }
      for (var i = 0, l = params.length; i < l; i++) {zy.log('----------22----------------'+JSON.stringify(params[i]));
