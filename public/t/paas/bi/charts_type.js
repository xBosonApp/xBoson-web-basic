(function () {
  ...
  function Event(con, chart, option, data, type, levellist, level) {
    ...
    option.delseries = function (index) {
      if (!index) return zy.log('删除指标时错误:缺少参数');
      var ta = [], series;
      series = this.series;
    ...
    option.showornot = function (index, flag) {
      if (!index) return zy.log('修改指标类型时错误:缺少参数');
  ...
  function BeforeEnd(option,extendo){
    ...
    zy.log('beforeInit',option);
    zy.log('String',JSON.stringify(option));
