...
Access_log = (function() {
  ...
  PT.Select = function() {
    // 预处理该画面所需的字典类型，以 , 号分割
    zy.cache.initDicts('ZR.0023',function(){
      searchForm.find('[name=access_cd]').zySelect('ZR.0023',false,{width:'100%'});
      // searchForm.find('[name=log_error_type]').zySelect('ZR.0024',false,{width:'100%'});
    });
    // 预处理APP select2
    zy.g.am.app = "cfb82858dc0a4598834d356c661a678f";
    zy.g.am.mod = "log_access";
    zy.net.get('api/user_select',function(msg){
      if(msg){
        searchForm.find('[name=userid]').zySelectCustomData('',false,{
          width:'100%'
        },msg.result);  
      }
    });
