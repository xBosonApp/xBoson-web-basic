...
function Module(con, param, done) {
  ...
  var chart_form_div = con.siblings('.chart_form_div').hide();
  ...
  thiz.fn.UpdateDom = function (mod) {
    ...
    modtype && modtype.select2('val',mod.modtype);
  ...
  thiz.fn.BuildSearch = function(search, searchparam, jformdiv){
      ...
      if (!jformdiv) {
        ...
        jformdiv = con.siblings('.chart_form_div');
      }
      jformdiv.empty();
      ...
      jformdiv.find(':input').each(function() {
        ...
        switch(type) {
          case 'select2_radio':
            ...
            var modolcd = thiz.attr('modolcd');
            if(dict=="" && modolcd!=""){
              zy.g.am.app='c770045becc04c7583f626faacd3b456';
              zy.g.am.mod='commapi';
              zy.net.get("api/exc_select", function(msg){
                $(thiz).zySelectCustomData("", true, {width: '100%'}, msg.data, false);
              },{modolcd:modolcd});
            } else {
  ...
  function Init(param, list, level) {
    ...
    thiz.fn.GetHTML(function (conn) {
      ...
      thiz.fn.GetModule(modid, function (module) {
        ...
        ownsearchparam = module.jsondata!=""?JSON.parse(module.jsondata).searchparam:{};
...
function Chart(con, newType, modulemsg, modolmg, module) {
  ...
  function Echart(newType, modulemsg, modolmg) {
    var chart, option,searchfield,axisDataFlag;
    ...
    if(axisDataFlag === 0) {
      // 判断x是否为类目轴
      option.chartoption&&option.chartoption.xAxis.forEach(function(inner){
