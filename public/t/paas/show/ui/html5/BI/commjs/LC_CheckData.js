	...
	// 货币
	var moneyRe=/(^[+-]?[1-9]\d*(\.\d{1,2})?$)|(^[+-]?[0]{1}(\.\d{1,2})?$)/ ;  // /^d*.?d{1,2}$/;
	var moneyErrorMsg ="输入的钱数格式不正确";
	
	// 负货币格式
	var moneyReNeg=/(^[-]?[1-9]\d*(\.\d{1,2})?$)|(^[-]?[0]{1}(\.\d{1,2})?$) /;    // /^-?d*.?d{1,2}$/;
	var moneyErrorMsgNeg ="只能输入小于0的钱数";
	
	// 正货币格式
	var moneyRePos=/(^[+]?[1-9]\d*(\.\d{1,2})?$)|(^[+]?[0]{1}(\.\d{1,2})?$)/ ;  // --  /^d*.?d{1,2}$/;
	var moneyErrorMsgPos ="只能输入大于0的钱数";
	// 浮点型
	...
	// 验证最小长度
	function sys_IsCheckLengthMin(id,itemsList,lenMin){
		if (!this.sys_IsCheckRe(lenMin,numReLen)) { 
   			retCheckFlg= false;
	...
	// 验证是否为空
	function sys_IsCheckNotNull(id,itemsList){
		itemsCheckNotNull(id,itemsList,notNullErrorMsg);
	}
	// 验证最大长度
	function sys_IsCheckLengthMax(id,itemsList,lenMax){
		if (!this.sys_IsCheckRe(lenMax,numReLen)) { 
   			retCheckFlg= false;
   			self.parent.$.messager.alert('错误提示','字符长度只能是正整数!','error');
   		}else{
	...
	// 验证最大长度和最下长度
	function sys_IsCheckLength(id,itemsList,lenMin,lenMax){
		if (!this.sys_IsCheckRe(lenMax,numReLen)||!this.sys_IsCheckRe(lenMin,numReLen)) { 
   			retCheckFlg= false;
   			self.parent.$.messager.alert('错误提示','字符长度只能是正整数!','error');
   		}else{
	    ...
	    if(itemsList.length>0){
		   	 ...
		   	 for(var i=0;i<itemsSize; i++){
					...
					if($("#"+id).find("[id='"+itemsList[m]+"']:eq("+i+")").val()!==""){
						if (!this.sys_IsCheckRe($("#"+id).find("[id='"+itemsList[m]+"']:eq("+i+")").val(),re)) { 
							retCheckFlg= false;
	...
	// 自定义
	function itemsCheckCus(id,itemsList,msg,re){
	//alert("re"+re);
	    // 如果数组不等于空
	    if(itemsList.length>0){
		   	clearError(id,itemsList[0],0);
			if($("#"+id).find("[id='"+itemsList[0]+"']:eq("+0+")").val()!==""){
				if (!this.sys_IsCheckRe1($("#"+id).find("[id='"+itemsList[0]+"']:eq("+0+")").val(),re)) { 
					 retCheckFlg= false;
                     setCheckError(id,itemsList[0],0,msg);
				}
			}
	    }
	}
	    ...
	    if(itemsList.length>0){
		   	 ...
		   	 for(var i=0;i<itemsSize; i++){
		   		 ...
		   		 for(var m=0;m<itemsList.length; m++){
		   			clearError(id,itemsList[m],i);
		   			if (lenMin>$("#"+id).find("[id='"+itemsList[m]+"']:eq("+i+")").val().replace(/[^\x00-\xff]/g,"xx").length) { 
                    //if (lenMin>$("#"+id).find("[id='"+itemsList[m]+"']:eq("+i+")").val().length) 		   			
					retCheckFlg= false;
		   				setCheckError(id,itemsList[m],i,msg);
		   			}
		   		 }
	...
	// 验证
	function sys_IsCheckRe(str,re){

		return re.test(str); 
	}
	function sys_IsCheckRe1(str,re){
		var  a=new RegExp(re);
		return a.test(str); 
	}
	// 设置错误信息
...
//		$("#"+id).find("[id='"+itemsId+"']:eq("+num+")").validatebox('remove');
	...
	// 清除错误信息 jiarc临时加误删除
	function clearError1(id,itemsId,num){
		//if($("#"+id).find("[id='"+itemsId+"']:eq("+num+")").attr("checkFlg")=="true"){
		   $("#"+id).find("[id='"+itemsId+"']:eq("+num+")").attr("title","");
		   $("#"+id).find("[id='"+itemsId+"']:eq("+num+")").css("border","");
	//	}
