...
// 跨域提交
function sys_CrossAjaxPost(url, formid, callback, infoMap, itemsList){
    var parmeData=sys_CrossJsonData(formid, infoMap, itemsList)
    sys_Request.setname(parmeData, url);
    sys_Request.send(url,callback);
}

// 为第三方跨域提交  系统默认调用
function sys_AjaxPostThird(url,reCallBack,paramData){
    retData=undefined;
	$.ajax({
	    type: "post",
...
// 为自己系统提供跨域post提交
function sys_AjaxPost(url,reCallBack,paramData){
    retData=undefined;
	$.ajax({
	    type: "post",
        contentType : 'application/json;charset=utf-8',
	    ...
	    success : function(data) {
			   ...
			   retData = data;
			     ...
			     self.parent.$.messager.alert('错误提示',"传递参数有问题",'warning');//传递参数有问题（调用接口参数校验错误） 
                             //sys_SetMapError("form1",data.dataMap);				
			}else if(data.ret==="103"){
...
function sys_SqlAjaxPost(url,formid,reCallBack,info,itemsList){


   
    retData=undefined;
	var jsonData=sys_PostJsonData(formid,info,itemsList);
//alert(jsonData);
	$.ajax({
	   type: "post",
       contentType : 'application/json;charset=utf-8',
...
// ajaxget提交
function sys_AjaxGet(url,formid,reCallBack,info,itemsList,step,stepNumName){
	retData={};
	var jsonData=sys_GetJsonData(formid,info,itemsList);
	//if(debuggerflg==true){
	   //alert(url+"?"+"callback="+reCallBack+"&"+jsonData);
	//}
	$.ajax({
	   ...
	   success : function(data) {
	   		if(data.ret==="0"){
			    if(step!=undefined&&step!=""){
				   if(stepNumName!=undefined&&stepNumName!=""){
						if(stepNumName=="sys_stepNum"){
							sys_stepNum = step;
						 }else if(stepNumName=="sys_stepNum0"){
							sys_stepNum0 = step;
						 }else if(stepNumName=="sys_stepNum1"){
							sys_stepNum1 = step;
						 }else if(stepNumName=="sys_stepNum2"){
							sys_stepNum2 = step;
						 }else if(stepNumName=="sys_stepNum3"){
							sys_stepNum3 = step;
						 }else if(stepNumName=="sys_stepNum4"){
							sys_stepNum4 = step;
						 }else if(stepNumName=="sys_stepNum5"){
							sys_stepNum5 = step;
						 }
					 }else{
					    sys_stepNum = step;
...
// 错误信息设置(List)
function sys_SetError(formid,data){
...
// 错误信息设置(Map)
function sys_SetMapError(formid,jsonMap){
